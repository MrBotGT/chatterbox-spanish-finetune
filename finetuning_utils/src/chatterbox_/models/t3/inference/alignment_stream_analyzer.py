...
class AlignmentStreamAnalyzer:
    ...
    def step(self, logits, next_token=None):
        ...
        long_tail = self.complete and (A[self.completed_at:, -3:].sum(dim=0).max() >= 10) # 400ms (increased for Spanish)
        ...
        # Check for excessive token repetition (5x same token in a row for Spanish)
        token_repetition = (
            # self.complete and
            len(self.generated_tokens) >= 6 and
            len(set(self.generated_tokens[-5:])) == 1  # Increased to 5x for Spanish
        )

        if token_repetition:
            repeated_token = self.generated_tokens[-1]
            logger.warning(f"🚨 Detected 5x repetition of token {repeated_token}")
